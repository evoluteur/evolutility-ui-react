...
const createMarkup = (d: string | null | undefined) => ({
  __html: d
...
const doc = (d: string | null | undefined, path: string) => {
  if (!d) {
    return null;
  }
  return (
...
export interface FieldElemBrowseProps {
  fieldDef: Field;
  value?: unknown;
  icon?: string | null;
}

const FieldElemBrowse = memo(
  ({ fieldDef, value, icon }: FieldElemBrowseProps) => {
    ...
    if (fType === ft.textml) {
      ...
      return (
        <div
          ...
          style={{ height }}
          dangerouslySetInnerHTML={createMarkup(value as string)}
        />
      );
    }
    if (fType === ft.image && value) {
      fw = image(filesUrl + (value as string));
    } else if (fType === ft.doc) {
      fw = doc(value as string, filesUrl);
    } else if (fType === ft.lov) {
      const lovValue = value as LovValue | undefined;
      if (f.object) {
        fw = (
          <Link to={`/${f.object}/browse/${lovValue?.id}`}>
