...
const prepData = (m: Model, data: RecordData): string => {
  let d = "{";
  m.fields.forEach((f) => {
    let v = data[f.id];
    if (v !== undefined) {
...
const fullCount = (qid: string) =>
  `_full_count: ${qid}_aggregate ${aggregateCount}`;
const filteredCount = (qid: string, filters: string) =>
  `_filtered_count: ${qid}_aggregate(${filters}) ${aggregateCount}`;

const sqFieldCol = (f: Field): string => {
  if (f.type === ft.lov) {
...
export const qFields = (m: Model): string =>
  "id " + m.fields?.map(sqFieldCol).join(" ") + timestampFields;

const conjunct = (join: string, conditions: string[]) =>
  `{_${join}: [${conditions.join(", ")}]}`;
//#endregion
...
const orderClause = (
  ...
  let oField: string | undefined, oDirection: string | undefined;
...
export const qMany = (
  ...
  if (m) {
    ...
    if (options?.page) {
      gOpts.push("offset:" + Number(options.page) * pSize);
    }

    const gOptsFilter: string[] = [];
    let allFilters = "";
      ...
      Object.keys(options).forEach((opt) => {
        if (opt === "page" || opt === "pageSize" || opt === "order") {
          ...
          if (f) {
            ...
            const params = String(options[opt]).split(".");
...
const aggregateName = (qid: string, f: Field): string => {
  if (f.aggregate) {
    return f.aggregate;
  } else {
...
export const qChart = (m: Model, fieldId: string): string | null => {
...
export const qStats = (m: Model): string => {
  // This query is expensiv so it has the @cached directive
  const sag: Record<string, string[]> = {};
  const nulls: string[] = [];
...
export const sqCollecs = (m: Model): string =>
  m.collections
    ?.map(
      (c) =>
        ` ${c.id}(order_by:{${c.order || "id"}: asc}) { id ${(
          c.fields as Field[]
        )
          .map(sqFieldCol)
          .join(" ")}}`,
...
export const qOne = (
  entity: string,
  nextOrPrevious?: keyof typeof nextPrev,
): string | null => {
  const m = getModel(entity);
  if (m) {
    ...
    if (m._lovNoList && m._lovNoList.length > 0) {
...
export const qDelete = (entity: string): string => {
  const m = getModel(entity);
...
export const qUpdateOne = (entity: string, data: RecordData): string => {
  const m = getModel(entity);
  return `mutation($id:Int!) {
...
export const qInsertOne = (entity: string, data: RecordData): string => {
  const m = getModel(entity);
  return `mutation {
...
export const qObjectSearch = (entity: string, search?: string): string => {
  const m = getModel(entity);
...
const sqLOV = (model: Model, fid: string): string => {
...
export const qLOVs = (model: Model): string => {
