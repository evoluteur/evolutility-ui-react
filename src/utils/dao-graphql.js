...
const mLOVFields = entity => moMa.getModel(entity).fields.filter(f => f.type===ft.lov)
...
const daoGraphQL = {
                ...
                if(options){
                    Object.keys(options).forEach(opt => {
                        if(opt==='page'){
                            ...
                            if(f && f.type===ft.lov){
                                gOpts.push('order_by:{'+orderParams[0]+':{name:'+orderParams[1]+'}}')
                            }else{
                                gOpts.push('order_by:{'+orderParams[0]+':'+orderParams[1]+'}')
                            }
                        } else if(opt==='search'){
                            // do nothing
                            // debugger
    ...
    deleteOne: function(entity, id) {
        const m = moMa.getModel(entity)
        return fetch(config.apiPathGraphQL, gqlOptions(qDelete(m.qid, id)))
            .then(toJSON)
    },
    ...
    addOne: function(entity, data) {        
        const m = moMa.getModel(entity) 
        return fetch(config.apiPathGraphQL, gqlOptions(qInsertOne(entity, m.qid, data)))
            .then(toJSON)
            .then(data => {
                data.data = data.data.inserted && data.data.inserted.returning.length ? data.data.inserted.returning[0] : null
                return data
            })
    },
    ...
    updateOne: function(entity, id, data) {
        const m = moMa.getModel(entity)
        return fetch(config.apiPathGraphQL, gqlOptions(qUpdateOne(m.id, m.qid, id, data)))
            .then(toJSON)
            .then(data => {
                data.data = data.data.updated && data.data.updated.returning.length ? data.data.updated.returning[0] : null
                return data
            })
    },
    ...
    // get the models
    getModels: () => fetch(config.apiPathGraphQL, gqlOptions(qModels))
            .then(toJSON),
