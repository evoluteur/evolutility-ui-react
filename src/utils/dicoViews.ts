...
const view = (name: string): ViewInfo => ({
  id: name,
  label: i18n[name],
  icon: name,
});

export const views: Record<string, ViewInfo> = {
  browse: view("browse"),
  edit: view("edit"),
  list: view("list"),
  cards: view("cards"),
...
export const modelViewsAnalytics = (
...
export const modelViewsMany = (
  model: Model | null | undefined,
  all?: boolean,
): ViewInfo[] => {
  const vs: ViewInfo[] = [views.list, views.cards];
  if (all) {
