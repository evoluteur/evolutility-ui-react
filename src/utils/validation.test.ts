...
describe("validate tests", () => {
  test("check validate ok", () => {
    const model = {
      fields: [intField, decField, textField, emailField],
    } as Model;
    const data = {
      integer: 5,
      decimal: 2.5,
  ...
  test("check validate fail", () => {
    const model = { fields: [intField] } as Model;
    const data = {
      integer: "abc",
    };
...
describe("diffData tests", () => {
  ...
  test("check diffData w/ delta", () => {
    ...
    expect(diffData(model, data1, data1)).toBe(null);
    let deltaKeys = Object.keys(diffData(model, data1, data2) as object);
    expect(deltaKeys.length).toBe(1);
    expect(deltaKeys[0]).toBe("name");
    deltaKeys = Object.keys(diffData(model, data1, data3) as object);
